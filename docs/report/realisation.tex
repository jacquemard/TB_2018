...
\subsection{Agent}
Un agent a été développé, déployé sur la VM. Celui-ci permet de définir un intervalle auquel des images seront demandées à la caméra. Il permet donc de gérer la connexion à la caméra, mais aussi les pertes de connexions. A la réception d'une image, il permet de définir une méthode de traitement (décrite en section \ref{realisation.traitement} \nameref{realisation.traitement}). 
...
\textit{Python} permet d'écouter les logs émit par un module. Il est aussi possible de définir le niveau d'écoute. Par exemple, si celui-ci est défini comme étant \textit{WARNING}, tous les logs de ce type seront capturé, ainsi que tous les logs dont le niveau est supérieur (soit plus important). 
...
Ainsi, il est possible d'agir rapidement lors d'une perte de connexion à la caméra. Le log associé étant de type (\textit{ERROR}), un mail est envoyé. Si la caméra est reconnectée (\textit{WARNING}), un mail est aussi reçu.


\section{Traitement des images}\label{realisation.traitement}
\todo{Anonymisation}
\subsection{Sous-échantillonnage (\textit{Downsampling})}
